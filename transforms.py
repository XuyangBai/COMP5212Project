...
# -*- coding: utf-8 -*-
"""
Created on Sun Nov 25 19:52:50 2018
...
class Pad_cls(object):
    ...
    def __call__(self, img):
        img_new, label_new = self.padder(img)
        return img_new

class Pad3d_cls(object):
...
class RandomCrop_cls(object):
    ...
    def __init__(self, size):
        if len(size) == 3:
            self.cropper = RandomCrop3d_cls(size)
        elif len(size) == 2:
            self.cropper = RandomCrop2d_cls(size)
        else:
...
class RandomBlack_cls(object):
    ...
    def __init__(self, black_patch_size=None):
        ...
        if black_patch_size is None:
            ...
            self.blacker = RandomBlack2d_cls(black_patch_size)
...
#            self.blacker = RandomBlack3d(black_patch_size)
    ...
    def __call__(self, tensor):
        return self.blacker(tensor)
        

class RandomBlack2d_cls(object):
    """ Randomly set patches to zeor"""
    def __init__(self, black_patch_size=None):
        self.black_patch_size = black_patch_size
